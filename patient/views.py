...
def postsign(request):
    ...
    try:
        patient= authe.sign_in_with_email_and_password(email,password)
    except:
        message = "invalid cerediantials"
        return render(request,"signIn.html",{"msg":message})
    print(patient['idToken'])
    session_id=patient['idToken']
    request.session['uid']=str(session_id)
    idtoken= request.session['uid']
    uid = patient['localId']
    #nam = database.child('patient').child(uid).child('name').get().val()
    return render(request, "welcome.html",{n:"nam"})   
def logout(request):
    auth.logout(request)
    return render(request,'signIn.html')
def signUp(request):
...
def postsignup(request):
    ...
    password=request.POST.get('password')
    try:
       patient=authe.create_user_with_email_and_password(email,password)
       uid=patient['localId']
       data={"name":name,"email":email,"age":age,"doc_name":doc_name}
